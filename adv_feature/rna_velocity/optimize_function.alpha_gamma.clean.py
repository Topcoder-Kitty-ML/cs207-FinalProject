...
def optimize_gamma(alpha, gamma, u_0, s_0, t_curr, u_t_actual, s_t_actual, learning_rate = 10000000):
	'''
	...
	loss_der_gamma = jp_matrix[0][0][0]
	...
	while True and iterations < iterations_cutoff:
		new_gamma = curr_gamma - learning_rate * float(loss_val / loss_der_gamma)
		...
		loss_der_gamma = jp_matrix[0][0][0]
		loss_val = jp_matrix[1][0][0]
		curr_gamma = new_gamma

		if(abs(loss_val - previous_error) < 10 **(-10)):
			break

		previous_error = loss_val
		iterations += 1
