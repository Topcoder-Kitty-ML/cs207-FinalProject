...
# Read data in
data = np.load("processed_data/norm_filtered_cells.pickle", allow_pickle=True)
...
def optimization_function(data, alpha_vals, gamma_vals):
	...
	for curr_gene_idx in range(num_genes):
		...
		dist_gene = calc_euclidian_distance(vals_predicted, vals_actual).val / len(vals_predicted)
...
print("dist is:", optimization_function(data, alpha_vals, gamma_vals))
...
# u_t = lambda alpha, u_0, t: alpha * (1 - math.exp(-1 * t)) + u_0 * math.exp(-1 * t)


# s_t = lambda alpha, gamma, u_0, s_0, t: (math.exp((-1*t) * (1+gamma)) * (math.exp(t*(1+gamma)) * \
# 			alpha * (gamma-1) + \
...
# curr_gamma = gamma
# while True:
# 	new_gamma = curr_gamma - learning_rate * float(val / der)
